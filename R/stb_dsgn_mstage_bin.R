...
get_postsample_bin_m = function(lst_design, ...){
  ...
  if (lst_design$borrow == 'control'){
    ...
    for(m in seq_len(max(gendat$stage))){
      ...
      sample_post_c = rbeta(lst_design$n_post, postd_c[2, k], postd_c[3, k])
      #treatment
      postd_t = postmix(postd_t, n = sst[m], r=rpst[m])
      k = sample.int(ncol(postd_t), lst_design$n_post, replace = TRUE, prob = postd_t[1,])
      sample_post_t = rbeta(lst_design$n_post, postd_t[2, k], postd_t[3, k])
      #diff
